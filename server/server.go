...
func NewServer(version string, apiKey string) *Server {
	s := &Server{
		version:    version,
		apiKey:     apiKey,
...
func (s *Server) registerTools() {
	...
	s.mcpServer.AddTool(mcp.NewTool("get-quote",
		mcp.WithDescription("Get a quote for swapping or bridging tokens. This is the PRIMARY tool for initiating any token swap. Returns the best route including expected output amount, fees, estimated time, and a transactionRequest object that can be passed to execute-quote. For ERC20 tokens, you may need to call approve-token first if allowance is insufficient."),
	...
	s.mcpServer.AddTool(mcp.NewTool("get-status",
		...
		mcp.WithString("txHash", mcp.Description("The transaction hash from the source chain (the hash returned by execute-quote)."), mcp.Required()),
	...
	), s.getGasSuggestionHandler)

	// LiFi API tools - API Key Testing
	s.mcpServer.AddTool(mcp.NewTool("test-api-key",
		mcp.WithDescription("Test if the configured LI.FI API key is valid. Returns key status and rate limit information. Requires API key to be configured via LIFI_API_KEY env var or --api-key flag."),
	), s.testApiKeyHandler)
	...
	// Blockchain interaction tools - Balance & Allowance Queries
	...
	s.mcpServer.AddTool(mcp.NewTool("get-allowance",
		mcp.WithDescription("Check how many ERC20 tokens a spender is approved to use on behalf of an owner. IMPORTANT: Before executing a swap with ERC20 tokens, verify the allowance is >= the swap amount. If insufficient, call approve-token first. The spender address for LI.FI swaps is returned in the get-quote response."),
		mcp.WithString("chain", mcp.Description("Chain identifier - either numeric ID (e.g., '1' for Ethereum) or name (e.g., 'ethereum'). The RPC URL is looked up automatically."), mcp.Required()),
		mcp.WithString("rpcUrl", mcp.Description("Optional: Custom RPC endpoint URL. Overrides the default RPC for the chain.")),
...
// These tools are only available when the server is started with --keystore flag
func (s *Server) RegisterWalletTools() {
	// Wallet information
	s.mcpServer.AddTool(mcp.NewTool("get-wallet-address",
		mcp.WithDescription("Get the Ethereum address of the currently loaded wallet. This is the address that will be used as 'fromAddress' in swaps and the signer for all transactions. Returns the address derived from the private key loaded via --keystore."),
	), s.getWalletAddressHandler)

	// Transaction execution
	s.mcpServer.AddTool(mcp.NewTool("execute-quote",
		mcp.WithDescription("Sign and broadcast a swap transaction to the blockchain. WORKFLOW: 1) Call get-quote to get route and transactionRequest, 2) Check allowance with get-allowance, 3) If needed, call approve-token, 4) Call this function with the transactionRequest. Returns the transaction hash which can be tracked with get-status for cross-chain swaps."),
		mcp.WithString("chain", mcp.Description("Chain identifier for the SOURCE chain (where the swap originates) - either numeric ID (e.g., '1') or name (e.g., 'ethereum'). Must match the chain in the quote."), mcp.Required()),
		mcp.WithString("rpcUrl", mcp.Description("Optional: Custom RPC endpoint URL. Overrides the default RPC for the chain.")),
		mcp.WithObject("transactionRequest", mcp.Description("The complete transactionRequest object from get-quote or get-step-transaction response. Contains to, data, value, gasLimit, and other transaction parameters. Pass the entire object unchanged."), mcp.Required()),
	), s.executeQuoteHandler)

	// Token approval
	s.mcpServer.AddTool(mcp.NewTool("approve-token",
		mcp.WithDescription("Approve a smart contract to spend your ERC20 tokens. REQUIRED before swapping ERC20 tokens - the LI.FI contract needs permission to move your tokens. Check current allowance with get-allowance first. For maximum approval (never approve again), use a very large number. For exact approval (more secure), use the exact swap amount."),
		mcp.WithString("chain", mcp.Description("Chain identifier - either numeric ID (e.g., '1' for Ethereum) or name (e.g., 'ethereum'). The RPC URL is looked up automatically."), mcp.Required()),
		mcp.WithString("rpcUrl", mcp.Description("Optional: Custom RPC endpoint URL. Overrides the default RPC for the chain.")),
		mcp.WithString("tokenAddress", mcp.Description("ERC20 token contract address to approve (the 'fromToken' in your swap)."), mcp.Required()),
		mcp.WithString("spenderAddress", mcp.Description("Contract address to approve for spending. For LI.FI swaps, use transactionRequest.to from the get-quote response."), mcp.Required()),
		mcp.WithString("amount", mcp.Description("Amount to approve in the token's smallest unit (wei). Use the same value as 'fromAmount' in your swap, or a larger value for future swaps. Example: '1000000' for 1 USDC (6 decimals)."), mcp.Required()),
	), s.approveTokenHandler)

	// Direct transfers (not swaps)
	s.mcpServer.AddTool(mcp.NewTool("transfer-token",
		mcp.WithDescription("Transfer ERC20 tokens directly to another address (no swap, no bridge). Use this for simple token transfers on a single chain. For cross-chain transfers, use get-quote + execute-quote instead."),
		mcp.WithString("chain", mcp.Description("Chain identifier - either numeric ID (e.g., '1' for Ethereum) or name (e.g., 'ethereum'). The RPC URL is looked up automatically."), mcp.Required()),
		mcp.WithString("rpcUrl", mcp.Description("Optional: Custom RPC endpoint URL. Overrides the default RPC for the chain.")),
		mcp.WithString("tokenAddress", mcp.Description("ERC20 token contract address to transfer."), mcp.Required()),
		mcp.WithString("to", mcp.Description("Recipient wallet address (0x... format)."), mcp.Required()),
		mcp.WithString("amount", mcp.Description("Amount to transfer in the token's smallest unit. Example: '1000000000000000000' for 1 token with 18 decimals, '1000000' for 1 USDC (6 decimals)."), mcp.Required()),
	), s.transferTokenHandler)

	s.mcpServer.AddTool(mcp.NewTool("transfer-native",
		mcp.WithDescription("Transfer native cryptocurrency (ETH, MATIC, BNB, etc.) directly to another address. Use this for simple native token transfers. The amount is specified in wei (1 ETH = 10^18 wei)."),
		mcp.WithString("chain", mcp.Description("Chain identifier - either numeric ID (e.g., '1' for Ethereum) or name (e.g., 'ethereum'). The RPC URL is looked up automatically."), mcp.Required()),
		mcp.WithString("rpcUrl", mcp.Description("Optional: Custom RPC endpoint URL. Overrides the default RPC for the chain.")),
		mcp.WithString("to", mcp.Description("Recipient wallet address (0x... format)."), mcp.Required()),
		mcp.WithString("amount", mcp.Description("Amount to transfer in wei (smallest unit). Examples: '1000000000000000000' for 1 ETH/MATIC, '100000000000000000' for 0.1 ETH."), mcp.Required()),
